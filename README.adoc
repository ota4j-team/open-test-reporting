...
The link:schema/src/main/resources/org/opentest4j/reporting/schema/events.xsd[first format] is _event-based_ which is not only suitable for writing events to a file but also for streaming events over a local socket or network connection.
...
The link:schema/src/main/resources/org/opentest4j/reporting/schema/hierarchy.xsd[second format] is meant to be closer to existing _hierarchical_ representations of test results that users are familiar with.
