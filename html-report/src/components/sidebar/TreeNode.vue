...
const showChildren = computed(() => props.children.length > 0 && !treeState.nodes[props.node.id]?.collapsed)
...
<template>
  <div class="inline-flex -mt-0.5" v-if="treeState.isVisible(execution.nodeStatuses((node as TestNodeData)))">
    <div @click="treeState.toggleNode(node.id)" class="cursor-pointer self-center" v-if="children.length > 0">
      <ChevronRight v-bind="defaultIconProps" v-if="treeState.nodes[node.id]?.collapsed" />
    ...
    }" @click="selectAndExpandNode()" role="link" :aria-label="node.name">
